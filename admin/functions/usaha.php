...
class Usaha
{
    ...
    public function add($data)
    {
        if (!empty($data)) {
            ...
            $query = "SELECT * FROM `usaha` WHERE id_deskel = ? AND id_su = ? AND id_ku = ? AND LOWER(nm_usaha) = ? AND thn_pmtkn = ? AND jns_ush = ? AND LOWER(nmr_izin) = ? AND LOWER(nm_pemilik) = ? AND LOWER(alamat) = ? AND latitude = ? AND longitude = ? AND no_telpon = ?";
            ...
            if ($stmt) {
                ...
                if ($result->num_rows > 0) {
                    ...
                    $insert = $this->db->query(
                        "INSERT INTO usaha (id_datum,id_deskel,id_su,id_ku,nm_usaha,thn_pmtkn,jns_ush,nmr_izin,nm_pemilik,alamat,tng_kerja_lki,tng_kerja_prmpn,mdl_sendiri,mdl_luar,asset,omset,latitude,longitude,no_telpon,gambar,polygon,ktp,surat_izin_usaha) VALUES(0,'$id_deskel', '$id_su', '$id_ku','$nama_usaha','$tahun_pembentukan','$jenis_usaha','$no_izin', '$nama_pemilik','$alamat','$tk_laki','$tk_perempuan','$modal_sendiri','$modal_luar','$asset','$omset','$latitude','$longitude','$no_telpon','$gambar','$polygon','$ktp','$surat_izin_usaha')"
                    );
    ...
    public function update($data)
    {
        ...
        if (!empty($data)) {
            ...
            $update = $this->db->query(
                ...
                asset='$asset', omset='$omset', latitude='$latitude', longitude='$longitude',no_telpon='$no_telpon',gambar='$gambar',polygon='$polygon',ktp='$ktp',surat_izin_usaha='$surat_izin_usaha' WHERE id_datum='$id_datum'"
            );
